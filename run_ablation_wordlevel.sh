#!/usr/bin/env bash
set -euo pipefail
# ========= 消融实验: AttackAgent 只使用 Word-Level Recombination =========
# 与 run_pairwise_qwen3.sh 同结构, 仅:
#   1) METHOD 改为 wordlevel_only 标识, 结果文件名据此生成
#   2) Python 入口换为 Multi_Agent_Framework_WordLevelOnly.py
...
BATCH_SIZE="64"
NUM_STEPS="100"
METHOD="multi_agent_pairwise_eval_wordlevel_only"   # ← 消融标识
PYTHON_ENTRY="Multi_Agent_Framework_WordLevelOnly.py"   # ← 消融入口
...
RL_GAMMA="0.9"
RL_EPSILON="0.2"
RL_SAVE_PATH="results/rl_qtable_wordlevel_only.json"   # ← 与主实验隔离
...
TEMP=$(getopt -o h --long help,device:,python-bin:,qwen3-path:,split-info:,split:,num-samples:,batch-size:,num-steps:,method:,save-dir:,save-path:,previous-results:,resume,use-rl-controller,use-attack-combiner -n 'run_ablation_wordlevel.sh' -- "$@")
...
echo "== Ablation: Word-Level Recombination only =="
