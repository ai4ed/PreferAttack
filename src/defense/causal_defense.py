"""
Causal Tail-Consistency Defense (CTC) for PreferAttack.

Core idea (see CTC_DEFENSE_DESIGN.md): instead of asking "does this suffix look
adversarial?" (anomaly detection -- which all prior defenses do, and which fails
because PreferAttack explicitly optimizes for stealth), CTC asks "does the
...
# --------------------------------------------------------------------------- #
# Shared utilities (inlined to keep CTC dependency-light: no sklearn, no       #
...
@dataclass
class CausalTailConsistencyDefense:
    """
    CTC defense: flag a pairwise query as adversarial if the judge's preference
    ...
    def _encode(self, text: str) -> List[int]:
        if self.tokenizer is None:
            raise RuntimeError("CTC needs a tokenizer to do token-level truncation.")
        # HF tokenizer path
    ...
    def evaluate_sample(self, instruction: str, suffix: str,
                        ...
                        qid: str = "") -> Dict:
        """
        Run CTC on one (instruction, suffix, A, B) tuple.
    ...
    def evaluate_sample_v2(self, instruction_clean: str, suffix: str,
                           response_a: str, response_b: str,
                           qid: str = "") -> Dict:
        """v2: Position-bias-corrected CTC via truncation-baseline correction.
