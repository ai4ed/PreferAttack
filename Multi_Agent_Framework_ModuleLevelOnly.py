"""
消融实验脚本: Attack Agent 只使用 Module-Level Recombination 动作。
...
本脚本通过 monkey-patch AttackAgent.run_update, 强制只调用 Module-Level 动作,
...
from utils.opt_utils import multi_granularity_hybrid


def run_update_module_level_only(self, *, params, search_mode, population, score_list,
                                 word_dict, num_elites_used, batch_size, crossover_used,
                                 mutation_used, topk_used, reference):
    """强制只使用 Module-Level Recombination (multi_granularity_hybrid),
    忽略 params['strategy_mode'] 与 search_mode。"""
...
MAF.AttackAgent.run_update = run_update_module_level_only


if __name__ == "__main__":
    print("[Ablation] AttackAgent restricted to Module-Level Recombination only "
          "(multi_granularity_hybrid).")
    MAF.main()
