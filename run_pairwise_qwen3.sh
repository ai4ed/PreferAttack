#!/usr/bin/env bash
set -euo pipefail
# ========= 在 run_pairwise_qwen3.sh 基础上改写为「串行多数据集」版本 =========
# 与原脚本 run_pairwise_qwen3.sh 的差异:
#   1) 新增 DATASETS 列表, 默认依次串行运行 code_judge_bench / alpaca_eval / arena_hard
#   2) 每个数据集独立生成结果文件 (METHOD_<dataset>_<TIMESTAMP>.json)
#   3) 多数据集模式下 RL Q-table 自动追加 _<dataset> 后缀, 避免互相覆盖
#   4) 单个数据集失败不会中断整轮 sweep, 最终以聚合状态退出
#   5) 仍兼容老用法: 用 --split-info PATH 退化为只跑单个数据集
# 原脚本 run_pairwise_qwen3.sh 保持不变。
...
# 默认串行运行的三个数据集 (每个对应 data/split/<ds>_split_info.json)
...
RL_GAMMA="0.9"
RL_EPSILON="0.2"
RL_SAVE_PATH="results/rl_qtable.json"   # 多数据集模式下自动追加 _<dataset> 后缀
...
TEMP=$(getopt -o h --long help,device:,python-bin:,qwen3-path:,split-info:,split:,num-samples:,batch-size:,num-steps:,method:,save-dir:,save-path:,datasets:,previous-results:,resume,use-rl-controller,use-attack-combiner -n 'run_pairwise_qwen3_datasets.sh' -- "$@")
...
while true; do
  case "$1" in
    ...
    -h | --help )
      echo "Usage: $0 [--device N] [--python-bin PATH] [--qwen3-path PATH]"
      echo "           [--datasets 'ds1 ds2 ds3'] [--split-info PATH] [--split test|train]"
      echo "           [--num-samples N] [--batch-size N] [--num-steps N]"
      echo "           [--method NAME] [--save-dir DIR] [--save-path PATH] [--previous-results PATH] --resume"
      echo "Default DATASETS: code_judge_bench alpaca_eval arena_hard (run serially)"
      exit 0 ;;
    -- ) shift; break ;;
    * ) break ;;
  esac
...
echo "== Running multi-agent pairwise with local Qwen3 (serial datasets) =="
echo "device=${device}  ->  CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
echo "Model path: ${QWEN3_PATH}"
echo "Datasets (serial): ${RUN_DATASETS}"
...
run_one() {
  ...
  if [ "${SAVE_PATH_USER_PROVIDED}" = "true" ]; then
    ...
    save_path="${SAVE_DIR}/${METHOD}_${dataset_name}_${ts}.json"
  fi

  # per-dataset RL Q-table: 多数据集模式下追加 _<dataset>, 避免互相覆盖
  local rl_save_path="${RL_SAVE_PATH}"
  if [ "${MULTI_MODE}" = "true" ]; then
    rl_save_path="${RL_SAVE_PATH%.json}_${dataset_name}.json"
  fi
  ...
  if [ ! -f "${split_info}" ]; then
    echo "[WARN] split_info not found: ${split_info} — skipping ${dataset_name}"
    return 3
  fi
  ...
  printf "%s\tstart=%s\tend=%s\telapsed_sec=%d\n" "${save_path}" "${start_ts}" "${end_ts}" ${elapsed} >> "${SAVE_DIR}/run_time.txt"
...
for ds in ${RUN_DATASETS}; do
  if run_one "${ds}"; then
    echo "[OK] dataset ${ds} completed"
  else
    s=$?
    echo "[FAIL] dataset ${ds} exited with status ${s} — continuing to next dataset"
    overall_status=${s}
  fi
done

echo ""
echo "All datasets finished. Overall exit status: ${overall_status}"
exit ${overall_status}
